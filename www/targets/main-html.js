...
flambe.display.TextAlign = $hxClasses["flambe.display.TextAlign"] = { __ename__ : true, __constructs__ : ["Left","Center","Right"] }
...
flambe.script.Parallel.prototype = {
	...
	,update: function(dt,actor) {
		var done = true;
		var maxSpent = 0.0;
		var _g1 = 0, _g = this._runningActions.length;
		while(_g1 < _g) {
			var ii = _g1++;
...
neobird.flambe.button.ButtonBase = function(handler,parameters) {
	...
	if(null == parameters) this._onClickParams = []; else this._onClickParams = parameters;
...
neobird.flambe.button.ButtonBase.prototype = $extend(flambe.display.Sprite.prototype,{
	...
	,onBtnMouseUp: function(upEvent) {
		if(flambe.System._platform.getTouch().get_supported()) this._status = neobird.flambe.button.ButtonStatus.DEFAULT; else this._status = neobird.flambe.button.ButtonStatus.OVER;
	...
	,onBtnMouseOver: function(overEvent) {
		if(flambe.System._platform.getMouse().isDown(flambe.input.MouseButton.Left) || flambe.System._platform.getTouch().get_points().length > 0) this._status = neobird.flambe.button.ButtonStatus.PRESSED; else this._status = neobird.flambe.button.ButtonStatus.OVER;
...
neobird.flambe.button.StdButton = function(imgName,pack,handler,parameters,checkTransparency) {
	...
	if(checkTransparency) this._imageEntity = new flambe.Entity().add(this._displayImg).add(new neobird.flambe.display.TransparencyCheck()); else this._imageEntity = new flambe.Entity().add(this._displayImg);
};
...
neobird.flambe.button.StdButton.prototype = $extend(neobird.flambe.button.ButtonBase.prototype,{
	...
	,set_text: function(text) {
		if(null == this._textdisplay) this.addTextDisplay(null,text); else this._textdisplay.set_text(text);
		return text;
	}
	,set_font: function(font) {
		if(null == this._textdisplay) this.addTextDisplay(font); else this._textdisplay.set_font(font);
		return font;
	}
...
neobird.flambe.button.ActivateButton.prototype = $extend(neobird.flambe.button.StdButton.prototype,{
	...
	,set_active: function(active) {
		if(active) this._displayImg = this._imgActive; else this._displayImg = this._img;
		this.updateDisplay();
		this.active = active;
		return this.active;
	}
...
neobird.flambe.button.ButtonManager.prototype = $extend(flambe.Component.prototype,{
	...
	,onAdded: function() {
		...
		if(flambe.System._platform.getTouch().get_supported()) {
		} else this._disposer.connect1(flambe.System._platform.getPointer().move,$bind(this,this.onMouseMoved));
	}
	,_disposer: null
	,_touchButtonPos: null
	,_overButton: null
...
neobird.flambe.button.ButtonStatus = $hxClasses["neobird.flambe.button.ButtonStatus"] = { __ename__ : true, __constructs__ : ["DEFAULT","OVER","PRESSED"] }
...
neobird.flambe.display.TransparencyCheck.prototype = $extend(flambe.Component.prototype,{
	...
	,hitTest: function(localX,localY) {
		...
		if(js.Boot.__instanceof(texture,flambe.platform.html.WebGLTexture)) return !this.isTransparentWebGL(texture,localX,localY); else if(js.Boot.__instanceof(texture,flambe.platform.html.CanvasTexture)) return !this.isTransparentCanvas(texture,localX,localY);
		return false;
	}
	,set_alphaThreshold: function(threshold) {
	...
	if(!neobird.flambe.sound.SoundManager._calledInit) {
		...
		flambe.System.hidden.get_changed().connect(function(hidden,_) {
			if(neobird.flambe.sound.SoundManager._musicChannel != null) {
				if(hidden) neobird.flambe.sound.SoundManager._musicChannel.pause(); else neobird.flambe.sound.SoundManager._musicChannel.resume();
			}
		});
	}
}
...
neobird.flambe.util.Metrics.init = function(accountId,gameName,domainName) {
	if(domainName == null) domainName = "neobirdgames.com";
	if(!neobird.flambe.util.Metrics._calledInit) {
		neobird.flambe.util.Metrics._calledInit = true;
		neobird.flambe.util.Metrics._gaTracker = new googleAnalytics.Tracker(accountId,domainName);
		neobird.flambe.util.Metrics._visitor = new googleAnalytics.Visitor();
		neobird.flambe.util.Metrics._visitor.setUserAgent(js.Browser.window.navigator.userAgent);
		neobird.flambe.util.Metrics._visitor.setScreenResolution(flambe.System._platform.getStage().get_width() + "x" + flambe.System._platform.getStage().get_height());
		neobird.flambe.util.Metrics._session = new googleAnalytics.Session();
		var page = new googleAnalytics.Page("/");
		page.setTitle(gameName);
		neobird.flambe.util.Metrics._gaTracker.trackPageview(page,neobird.flambe.util.Metrics._session,neobird.flambe.util.Metrics._visitor);
	}
}
neobird.flambe.util.Metrics.trackEvent = function(category,action,label,value) {
	var event = new googleAnalytics.Event(category,action,label,value);
	neobird.flambe.util.Metrics._gaTracker.trackEvent(event,neobird.flambe.util.Metrics._session,neobird.flambe.util.Metrics._visitor);
}
neobird.flambe.util.Metrics.trackError = function(action,label,value) {
	neobird.flambe.util.Metrics.trackEvent("error",action,label,value);
}
neobird.sudoku = {}
...
neobird.sudoku.brandingscreen.BrandingScreen.prototype = $extend(flambe.scene.Scene.prototype,{
	...
	,onAdded: function() {
		...
		if(this._playSound) neobird.flambe.sound.SoundManager.get_soundQueue().loadFile({ id : "snd_branding", src : "assets/sounds/ZibboMix64.ogg|assets/sounds/ZibboMix64.mp3"}); else this.startAnimation();
	}
	,_playSound: null
	,_movie: null
	,_manager: null
...
neobird.sudoku.game.AssetData.initAssetData = function() {
	...
	var _g = 0, _g1 = neobird.sudoku.game.AssetData.ASSETDATA;
	while(_g < _g1.length) {
		var asset = _g1[_g];
		++_g;
...
neobird.sudoku.game.Difficulty = $hxClasses["neobird.sudoku.game.Difficulty"] = { __ename__ : true, __constructs__ : ["EASY","MEDIUM","DIFFICULT"] }
...
neobird.sudoku.game.GameManager = function() {
	...
	if(Math.max(flambe.System._platform.getStage().get_width(),flambe.System._platform.getStage().get_height()) < 1500) {
...
neobird.sudoku.gamescreen.Playfield.prototype = $extend(flambe.display.Sprite.prototype,{
	validateBlock: function(col,row) {
		...
		var _g1 = blockX, _g = blockX + 3;
		while(_g1 < _g) {
			var i = _g1++;
			var _g3 = blockY, _g2 = blockY + 3;
			while(_g3 < _g2) {
				var j = _g3++;
...
neobird.sudoku.gamescreen.Tile.prototype = $extend(flambe.display.TextSprite.prototype,{
	adjustColor: function() {
		if(!this.valid) {
			if(this.pattern) this.set_font(this._manager.playfield_pattern_wrong); else this.set_font(this._manager.playfield_user_wrong);
		} else if(this.pattern) this.set_font(this._manager.playfield_pattern); else this.set_font(this._manager.playfield_user);
	}
	...
	,set_value: function(value) {
		this.value = value;
		if(value > 0) this.set_text(Std.string(value)); else this.set_text("");
		return value;
	}
	,valid: null
	,pattern: null
...
neobird.sudoku.preloader.Preloader = function(manager) {
	...
	this._soundManifest = [{ id : "music", src : "assets/sounds/ephemeralrift.ogg|assets/sounds/ephemeralrift.mp3"},{ id : "wrong", src : "assets/sounds/hokshrt4.ogg|assets/sounds/hokshrt4.mp3"},{ id : "btn_press", src : "assets/sounds/snd_letters_click_4.ogg|assets/sounds/snd_letters_click_4.mp3"}];
...
neobird.sudoku.solver.RandomIterator = function() {
	...
	var _g1 = 0, _g = neobird.sudoku.game.Constants.DIM_BOARD;
	while(_g1 < _g) {
		var i = _g1++;
		available.push(i);
	}
...
neobird.sudoku.solver.BacktrackSolver.prototype = {
	...
	,hasMultipleSolutions: function(squares,candidate) {
		...
		this.purge(squares);
		var _g1 = 1, _g = neobird.sudoku.game.Constants.DIM_NUMBERS + 1;
		while(_g1 < _g) {
			var i = _g1++;
			if(i == value) continue;
	...
	,violatesRestrictions: function(squares,candidate,givenLimit,givenLimitTotal) {
		...
		var _g1 = 0, _g = neobird.sudoku.game.Constants.DIM_REGION;
		while(_g1 < _g) {
			var i = _g1++;
			givenCol.push(0);
			givenRow.push(0);
	...
	,solve: function(squares) {
		this.purge(squares);
		var boardPos = 0;
		while(boardPos < neobird.sudoku.game.Constants.DIM_BOARD) if(squares[boardPos].pickValue()) {
	...
	,generate: function(sudoku,difficulty) {
		...
		while(_g < _g1.length) {
			var square = _g1[_g];
			++_g;
			if(square.pattern) sudoku[square.col][square.row] = square.value; else sudoku[square.col][square.row] = 0;
		}
	}
	,_terminal: null
...
flambe.asset.Manifest.__meta__ = { obj : { assets : [{ sounds : [{ bytes : 1524924, md5 : "625de89ee9a4a9c424c55b54698e27bb", name : "133726__ephemeral-rift__native-american-flute-in-d-minor-and-stream.mp3"},{ bytes : 2195290, md5 : "0083852c6d0e5d7f9bde7221072c934d", name : "133726__ephemeral-rift__native-american-flute-in-d-minor-and-stream.ogg"},{ bytes : 15360, md5 : "f21092d3b0771a56b302a891cb267ba3", name : "81530__johnlavine333__hokshrt4.mp3"},{ bytes : 21336, md5 : "47b92d1e56a232c17d7bd2f54e60ef51", name : "81530__johnlavine333__hokshrt4.ogg"},{ bytes : 546377, md5 : "23563c8269b9e716f17a83564f352b48", name : "ephemeralrift.mp3"},{ bytes : 489880, md5 : "4c6d6825ebd77a8fc27676ffa356dd0f", name : "ephemeralrift.ogg"},{ bytes : 15360, md5 : "f21092d3b0771a56b302a891cb267ba3", name : "hokshrt4.mp3"},{ bytes : 21336, md5 : "47b92d1e56a232c17d7bd2f54e60ef51", name : "hokshrt4.ogg"},{ bytes : 1724, md5 : "83bf28d40c7625dcc4d2ed38ce44d53b", name : "snd_letters_click_4.mp3"},{ bytes : 5780, md5 : "7c4e294dbf2d3c773401f490f5eb3766", name : "snd_letters_click_4.ogg"},{ bytes : 32448, md5 : "bac60e0930bc1c4718e1991506c0bc18", name : "ZibboMix64.mp3"},{ bytes : 42965, md5 : "5a8d9a4253244a8c77b58dbe5d7dc5e3", name : "ZibboMix64.ogg"}], bootstrap : [{ bytes : 3950130, md5 : "124eeb151ce5c91b38de4468f5731be4", name : "Background.png"},{ bytes : 10670, md5 : "82e6c8939786caca494c7bd8d22c6a6d", name : "branding/atlas0.png"},{ bytes : 12170, md5 : "4ad55a87bcb94d1d1b8cbe8226a6e702", name : "branding/library.json"},{ bytes : 25951, md5 : "ab33e1f3c1f326e72202509fde59f2d6", name : "fonts/arial_std.fnt"},{ bytes : 51439, md5 : "f4f143a0a081b23f4e9af135758d90f4", name : "fonts/arial_std_0.png"},{ bytes : 24147, md5 : "12569807f2872e60d516941930a09e49", name : "fonts/ginko_small.fnt"},{ bytes : 65143, md5 : "62b65906e579dd0b111a2f994804b927", name : "fonts/ginko_small_0.png"},{ bytes : 24149, md5 : "6796a600a09de70dc12f54d40ae225d4", name : "fonts/ginko_std.fnt"},{ bytes : 132457, md5 : "df673d9453a7a285b8390d82a2411f11", name : "fonts/ginko_std_0.png"},{ bytes : 1618, md5 : "1b2f3516ccb49361163679fc9be20e33", name : "fonts/playfield_pattern.fnt"},{ bytes : 9735, md5 : "0139d8b2266ea1d51e947dfa7ba61b79", name : "fonts/playfield_pattern_0.png"},{ bytes : 1624, md5 : "726a79195de6feba538077ded3179087", name : "fonts/playfield_pattern_wrong.fnt"},{ bytes : 9735, md5 : "0139d8b2266ea1d51e947dfa7ba61b79", name : "fonts/playfield_pattern_wrong_0.png"},{ bytes : 1615, md5 : "4a381c02342a56b60f1e9d93f45f041e", name : "fonts/playfield_user.fnt"},{ bytes : 9676, md5 : "26b000c395e8e6ecbd6feb0b5f2c919f", name : "fonts/playfield_user_0.png"},{ bytes : 1621, md5 : "610c0e613a35015209959c05a3dfd7fa", name : "fonts/playfield_user_wrong.fnt"},{ bytes : 9759, md5 : "d8f166e64c0e65846d3f17add3a305ef", name : "fonts/playfield_user_wrong_0.png"},{ bytes : 190278, md5 : "ad50f95b89d4e289ab75638b59950a3c", name : "Headline.png"},{ bytes : 1722642, md5 : "721085aa4a9f11b1afcba4143f9d44ac", name : "Helpscreen.png"},{ bytes : 26276, md5 : "b6cd51783c2f6630e9b7ccdad9cce80b", name : "MainMenuButton.png"},{ bytes : 24119, md5 : "99d96830fa31a7d1de5b187481ce663a", name : "MainMenuButtonActive.png"},{ bytes : 24989, md5 : "9d22821bba31e705452582297250ce42", name : "MainMenuButtonMouseover.png"},{ bytes : 24617, md5 : "bd8d85649b7de497fe7321f30811d896", name : "MainMenuButtonPressed.png"},{ bytes : 348244, md5 : "01348c7a8fcd7068a37341fc60ec7bef", name : "MobileDeviceTurnAround.png"},{ bytes : 45159, md5 : "193e8b0c71a108f489e75677221f689e", name : "NeobirdLogo.png"},{ bytes : 15862, md5 : "5bf7b0a0ba6c58e2ae17ece2927a9342", name : "ZibboLogo.png"}], 'bootstrap.android' : [{ bytes : 1260317, md5 : "bf20d043443c8ae1f592ce189c82ec0a", name : "Background.png"},{ bytes : 10670, md5 : "82e6c8939786caca494c7bd8d22c6a6d", name : "branding/atlas0.png"},{ bytes : 12170, md5 : "4ad55a87bcb94d1d1b8cbe8226a6e702", name : "branding/library.json"},{ bytes : 25909, md5 : "c6ded95539bbc311fef5e02845d4c984", name : "fonts/arial_std.fnt"},{ bytes : 21756, md5 : "f5eaa502335ee515881cbdc790088c6d", name : "fonts/arial_std_0.png"},{ bytes : 24147, md5 : "320f613c3047e2a6568895ec14a10280", name : "fonts/ginko_small.fnt"},{ bytes : 28042, md5 : "76993843a1678e66a7532bbc29e967cd", name : "fonts/ginko_small_0.png"},{ bytes : 24145, md5 : "4dc329062a1f054a61c57606177176a7", name : "fonts/ginko_std.fnt"},{ bytes : 57334, md5 : "d7cf412986957aa899112d3b0885e4c6", name : "fonts/ginko_std_0.png"},{ bytes : 1614, md5 : "df29babde1ec31419379f54eb86cfd35", name : "fonts/playfield_pattern.fnt"},{ bytes : 4675, md5 : "dab571e7c4d38850da455b69948ba969", name : "fonts/playfield_pattern_0.png"},{ bytes : 1620, md5 : "1222a08e09c04547aea1787d8aab687d", name : "fonts/playfield_pattern_wrong.fnt"},{ bytes : 4675, md5 : "677db274f0b36df4882f7520c616a9c1", name : "fonts/playfield_pattern_wrong_0.png"},{ bytes : 1611, md5 : "bce2ad497b31f53c66902576747df236", name : "fonts/playfield_user.fnt"},{ bytes : 4634, md5 : "b2b81d3834e72debafe52edc6c08d817", name : "fonts/playfield_user_0.png"},{ bytes : 1617, md5 : "add9d36d209377e29fbc9476872c5e5e", name : "fonts/playfield_user_wrong.fnt"},{ bytes : 4669, md5 : "0221c59642b94f69ca017bfde709b1f9", name : "fonts/playfield_user_wrong_0.png"},{ bytes : 96491, md5 : "062c07b2bdf0598c4d9d787a295daaa5", name : "Headline.png"},{ bytes : 841949, md5 : "032f54be6728206b3d900c6981356b9e", name : "Helpscreen.png"},{ bytes : 14947, md5 : "201a1a2d191cc1d6d15e2e4048375c26", name : "MainMenuButton.png"},{ bytes : 15758, md5 : "dc3d64f8fd3c5d01e275b5b9b076f160", name : "MainMenuButtonActive.png"},{ bytes : 16030, md5 : "81770b77d42efcdab44d1d17d5b46b79", name : "MainMenuButtonMouseover.png"},{ bytes : 15790, md5 : "03bacc1c1db0ed144b7c38a4aaef99bc", name : "MainMenuButtonPressed.png"},{ bytes : 117485, md5 : "1743ff929aba0929736fc0891813b30d", name : "MobileDeviceTurnAround.png"},{ bytes : 18467, md5 : "b2d97a395ff4e6f6063bcc5dfc2e525d", name : "NeobirdLogo.png"},{ bytes : 8575, md5 : "d743ef9093938ee9a98a546188454957", name : "ZibboLogo.png"}], 'game.android' : [{ bytes : 7075, md5 : "47dc15dce0ec404bf312f1addbdf1b6f", name : "buttons/BigButtonLeft.png"},{ bytes : 6718, md5 : "01321771e4ba0e2dc52c7b912d178ca2", name : "buttons/BigButtonLeftMouseover.png"},{ bytes : 7012, md5 : "4be16a4d42e5f195a92745ad81ba807d", name : "buttons/BigButtonLeftPressed.png"},{ bytes : 7085, md5 : "3a63f1c976b59f6e7b768a5e5f35ac2b", name : "buttons/BigButtonRight.png"},{ bytes : 6888, md5 : "f7785c7e07df0093ae579056f107848c", name : "buttons/BigButtonRightMouseover.png"},{ bytes : 7226, md5 : "747a7f7d63d9d4460e598b5c5d05be60", name : "buttons/BigButtonRightPressed.png"},{ bytes : 9033, md5 : "2ca80de35baf14d2bda790f46ea704a1", name : "buttons/Exit.png"},{ bytes : 9532, md5 : "b76155be7b0bc188337ce3923e986477", name : "buttons/ExitMouseover.png"},{ bytes : 9061, md5 : "afda9ff81cb3ab4c95002026c13c0414", name : "buttons/ExitPressed.png"},{ bytes : 15758, md5 : "dc3d64f8fd3c5d01e275b5b9b076f160", name : "buttons/MainMenuButton.png"},{ bytes : 16030, md5 : "81770b77d42efcdab44d1d17d5b46b79", name : "buttons/MainMenuButtonMouseover.png"},{ bytes : 15790, md5 : "03bacc1c1db0ed144b7c38a4aaef99bc", name : "buttons/MainMenuButtonPressed.png"},{ bytes : 10524, md5 : "c3b9edd9be387f790a7e9e2925b56971", name : "buttons/MoreGamesButton.png"},{ bytes : 10577, md5 : "727722c51c8a7f5077335c05646b68cc", name : "buttons/MoreGamesButtonMouseover.png"},{ bytes : 10596, md5 : "de0fe4fbb0b18990ba8a65068450286f", name : "buttons/MoreGamesButtonPressed.png"},{ bytes : 9319, md5 : "05419152398078bf86bf457846bcc9d5", name : "buttons/Music.png"},{ bytes : 9795, md5 : "43922efc4b6d059314e181ad3e4e427d", name : "buttons/MusicMouseover.png"},{ bytes : 9722, md5 : "cf27e0e77d9f83e2875ba9a9b17a6805", name : "buttons/MusicOff.png"},{ bytes : 10149, md5 : "afeb1eab4f9e8ffd36a6b59aaf584fa0", name : "buttons/MusicOffMouseover.png"},{ bytes : 9731, md5 : "f5e6514c6924573c0246ec8f53c4218d", name : "buttons/MusicOffPressed.png"},{ bytes : 9349, md5 : "3a64a93f9f8938997cd3b120bda32a1f", name : "buttons/MusicPressed.png"},{ bytes : 11004, md5 : "13e76c8a37561654185fdde0c5cb2216", name : "buttons/NewGameButton.png"},{ bytes : 11300, md5 : "0e3e382a6c06b0862db555678689f833", name : "buttons/NewGameButtonMouseover.png"},{ bytes : 11121, md5 : "f668ae243bfcdb70fe6c6f150a414379", name : "buttons/NewGameButtonPressed.png"},{ bytes : 5769, md5 : "cb038e17d19976112861b69214cd47ec", name : "buttons/SmallButtonLeft.png"},{ bytes : 5559, md5 : "29c5f568c4b860cb1d9e994dd7d0b8ec", name : "buttons/SmallButtonLeftMouseover.png"},{ bytes : 5766, md5 : "be2b98358801a8eb636ab01a3fb5722e", name : "buttons/SmallButtonLeftPressed.png"},{ bytes : 5840, md5 : "29b46a35f3fe4e731e7f7311292b430f", name : "buttons/SmallButtonRight.png"},{ bytes : 5732, md5 : "e3f762c6ca7fb96f05df1cafccb2a28d", name : "buttons/SmallButtonRightMouseover.png"},{ bytes : 5889, md5 : "ea714f5774e3e21af4fdb3bdbf884bc3", name : "buttons/SmallButtonRightPressed.png"},{ bytes : 9776, md5 : "3a4707ed44eb0762cc4ddc444f139c10", name : "buttons/Sound.png"},{ bytes : 10289, md5 : "e12971f2bd05cdc12da06f5be043be92", name : "buttons/SoundMouseover.png"},{ bytes : 9987, md5 : "03911aeba227a12433d7bb69d890b46b", name : "buttons/SoundOff.png"},{ bytes : 10467, md5 : "48dde4e340d2e58433dd0e42db9af5c2", name : "buttons/SoundOffMouseover.png"},{ bytes : 10023, md5 : "4858fbf764940de8eb30a56ab409cdb3", name : "buttons/SoundOffPressed.png"},{ bytes : 9825, md5 : "923babae224504cd1abf76a92905c2dd", name : "buttons/SoundPressed.png"},{ bytes : 9547, md5 : "d9df00fc3672ce3c3b812b8563a9b59b", name : "img/DifficultyBackground.png"},{ bytes : 9594, md5 : "07070d353fb42ee9ed3ce90e6726f2d3", name : "img/DifficultyBackgroundMain.png"},{ bytes : 69423, md5 : "7963a6558c3b2330ae93acb4b6e7144d", name : "img/Field.png"},{ bytes : 96491, md5 : "062c07b2bdf0598c4d9d787a295daaa5", name : "img/Headline.png"},{ bytes : 117771, md5 : "36ee13f34af6eb2c8350e7f9367ee981", name : "img/MobileDeviceTurnAround.png"},{ bytes : 337373, md5 : "90dea28de4520f31cb920ac880d1d47f", name : "img/MobileDeviceTurnAroundHuge.png"},{ bytes : 18467, md5 : "b2d97a395ff4e6f6063bcc5dfc2e525d", name : "img/NeobirdLogo.png"},{ bytes : 183861, md5 : "cf995f868eef4941bdfaf8c7234a013a", name : "img/PopUp.png"},{ bytes : 2942, md5 : "69a61d9a09c560d863c0959181dc9aab", name : "img/PopUpBackground.png"},{ bytes : 3352, md5 : "0e792a260ac8a154fe9b0489dfd04d62", name : "img/Selection.png"},{ bytes : 40063, md5 : "1e53a7d666a9c2879693a340bdae4f74", name : "img/SelectionBack.png"},{ bytes : 5905, md5 : "49f27cd7d43a436c980ffd1dd5457b4c", name : "img/SelectionMouseover.png"},{ bytes : 5905, md5 : "49f27cd7d43a436c980ffd1dd5457b4c", name : "img/SelectionPressed.png"},{ bytes : 143964, md5 : "93bea76229845d667649790b3f29a7a8", name : "img/SudokuPopUp.png"},{ bytes : 5138, md5 : "eecf77c4365c8869b18642644f451353", name : "img/SudokuSparkle.png"},{ bytes : 127579, md5 : "4c9359b8478d39bdb04db404acde7741", name : "img/WellDone.png"},{ bytes : 740425, md5 : "12d4d56daf5616c40cb7dee2fdaa52a2", name : "img/WinField.png"},{ bytes : 8575, md5 : "d743ef9093938ee9a98a546188454957", name : "img/ZibboLogo.png"},{ bytes : 117485, md5 : "1743ff929aba0929736fc0891813b30d", name : "MobileDeviceTurnAround.png"},{ bytes : 1260317, md5 : "bf20d043443c8ae1f592ce189c82ec0a", name : "screens/Background.png"},{ bytes : 841949, md5 : "032f54be6728206b3d900c6981356b9e", name : "screens/Helpscreen.png"}], game : [{ bytes : 12514, md5 : "46b3c3ade0b7fcfb28e4c21a9e51f5e7", name : "buttons/BigButtonLeft.png"},{ bytes : 11972, md5 : "49aff0f7d2215ddabb37d1c83ee251d4", name : "buttons/BigButtonLeftMouseover.png"},{ bytes : 12640, md5 : "da03ae745f6661bc63739ffec9545194", name : "buttons/BigButtonLeftPressed.png"},{ bytes : 12423, md5 : "e722ea8c039a52e4e3ff7053478b47fe", name : "buttons/BigButtonRight.png"},{ bytes : 12343, md5 : "f603a8e73f74491199687c41a83fabe7", name : "buttons/BigButtonRightMouseover.png"},{ bytes : 12621, md5 : "a1130f3193b543e9b6718a7282f1e2d3", name : "buttons/BigButtonRightPressed.png"},{ bytes : 15140, md5 : "58b7fbea7c59d815c5c234537049d268", name : "buttons/Exit.png"},{ bytes : 16385, md5 : "b01d870dd28f4f9f7a31f7ff50fce8b0", name : "buttons/ExitMouseover.png"},{ bytes : 15102, md5 : "54b0bcddd031ffd9dc9f26820c28a364", name : "buttons/ExitPressed.png"},{ bytes : 24119, md5 : "99d96830fa31a7d1de5b187481ce663a", name : "buttons/MainMenuButton.png"},{ bytes : 24989, md5 : "9d22821bba31e705452582297250ce42", name : "buttons/MainMenuButtonMouseover.png"},{ bytes : 24617, md5 : "bd8d85649b7de497fe7321f30811d896", name : "buttons/MainMenuButtonPressed.png"},{ bytes : 20208, md5 : "444d86e88525671ff0522a231bbc9f52", name : "buttons/MoreGamesButton.png"},{ bytes : 20650, md5 : "5bcf5445875e6388e38eacb18b4d1cf7", name : "buttons/MoreGamesButtonMouseover.png"},{ bytes : 20562, md5 : "b031bd04923f02fd36613a3dab5cdaf7", name : "buttons/MoreGamesButtonPressed.png"},{ bytes : 15853, md5 : "1877f84f1dfc005d3b464963212ac4f1", name : "buttons/Music.png"},{ bytes : 17155, md5 : "ae2ee5683c1dfab3b4111fd98a7c0a73", name : "buttons/MusicMouseover.png"},{ bytes : 17024, md5 : "6e08632f5811bafa304e85fa674adba0", name : "buttons/MusicOff.png"},{ bytes : 18153, md5 : "2ab88bfd26dcd368346b1d9c10937ccc", name : "buttons/MusicOffMouseover.png"},{ bytes : 17013, md5 : "e7c9fb3e42fd8c0e81be16a0dacc6f4d", name : "buttons/MusicOffPressed.png"},{ bytes : 15887, md5 : "b2864a60083d00316bed869e1ccd01fd", name : "buttons/MusicPressed.png"},{ bytes : 16741, md5 : "a46704e1ac093c4708a92c0116a3a9d0", name : "buttons/NewGameButton.png"},{ bytes : 17308, md5 : "aebe50ad6ad51a47f9785c0c2edbc0d9", name : "buttons/NewGameButtonMouseover.png"},{ bytes : 16977, md5 : "9f00c5d8ae3d521b7a05fd05fb9886cd", name : "buttons/NewGameButtonPressed.png"},{ bytes : 9817, md5 : "840970a63bd642a9975c51be7b9cfb44", name : "buttons/SmallButtonLeft.png"},{ bytes : 9602, md5 : "754e7bf1c67a76635715a898a9c13d08", name : "buttons/SmallButtonLeftMouseover.png"},{ bytes : 9668, md5 : "9bfce0cb864c536af857c0846f56632d", name : "buttons/SmallButtonLeftPressed.png"},{ bytes : 9689, md5 : "c72ad3e8d372a9bb444a1480b54658bf", name : "buttons/SmallButtonRight.png"},{ bytes : 9771, md5 : "62ed50ceac3797cdbf9fbd9346cf10a3", name : "buttons/SmallButtonRightMouseover.png"},{ bytes : 9910, md5 : "5b1116bb9a1f606cf9675830f86ada37", name : "buttons/SmallButtonRightPressed.png"},{ bytes : 17477, md5 : "7ff96a1c9f620763d0cbf4f6e1b5c746", name : "buttons/Sound.png"},{ bytes : 18770, md5 : "4c022e62be54329dced972b5cb431597", name : "buttons/SoundMouseover.png"},{ bytes : 18273, md5 : "d9d35048606fbffb3688bebc4ba5284d", name : "buttons/SoundOff.png"},{ bytes : 19366, md5 : "99768a46698a94c5315588c81df1f09c", name : "buttons/SoundOffMouseover.png"},{ bytes : 18230, md5 : "d204634c32d2a3553a7505b3706b1103", name : "buttons/SoundOffPressed.png"},{ bytes : 17412, md5 : "20ab025839ef8ab2ad71d04877c6f8a2", name : "buttons/SoundPressed.png"},{ bytes : 15068, md5 : "3f8d9bb3d6a457c6172385f3152b9558", name : "img/DifficultyBackground.png"},{ bytes : 14300, md5 : "b6f6f533202e6b8042f0a927ed6b1acc", name : "img/DifficultyBackgroundMain.png"},{ bytes : 138083, md5 : "a638b2325ba79403d6e37a31c5819fe8", name : "img/Field.png"},{ bytes : 190278, md5 : "ad50f95b89d4e289ab75638b59950a3c", name : "img/Headline.png"},{ bytes : 45159, md5 : "193e8b0c71a108f489e75677221f689e", name : "img/NeobirdLogo.png"},{ bytes : 574271, md5 : "1055584d098dc787ce28ed2967793a55", name : "img/PopUp.png"},{ bytes : 3077, md5 : "63afd7cca7316d48533875b683f0e393", name : "img/PopUpBackground.png"},{ bytes : 3002, md5 : "27e8d389822c78eabcc78003f0d2be99", name : "img/Selection.png"},{ bytes : 97522, md5 : "b7683cf1a277e84c6551bdd888b70ecc", name : "img/SelectionBack.png"},{ bytes : 8433, md5 : "c1eb79c6cb93ff460922dc2fa97d9544", name : "img/SelectionMouseover.png"},{ bytes : 8433, md5 : "c1eb79c6cb93ff460922dc2fa97d9544", name : "img/SelectionPressed.png"},{ bytes : 317795, md5 : "056e0699dbfb5e131fdccc6064690ec3", name : "img/SudokuPopUp.png"},{ bytes : 6430, md5 : "9221b1a30f1fbea6cdd32cda0e4c93d8", name : "img/SudokuSparkle.png"},{ bytes : 262345, md5 : "a7797a04bbbea0cdcf54f7b8dd2621a8", name : "img/WellDone.png"},{ bytes : 1551847, md5 : "0a62044d01a5b2a856a67cf1c3cb58b4", name : "img/WinField.png"},{ bytes : 15862, md5 : "5bf7b0a0ba6c58e2ae17ece2927a9342", name : "img/ZibboLogo.png"},{ bytes : 348244, md5 : "01348c7a8fcd7068a37341fc60ec7bef", name : "MobileDeviceTurnAround.png"},{ bytes : 3950130, md5 : "124eeb151ce5c91b38de4468f5731be4", name : "screens/Background.png"},{ bytes : 1722642, md5 : "721085aa4a9f11b1afcba4143f9d44ac", name : "screens/Helpscreen.png"}]}]}};
...
neobird.sudoku.game.AssetData.ASSETDATA = [{ id : "designres", x : 2272, y : 1536},{ id : "coreres", x : 1920, y : 1282},{ id : "stdBtnExtend", x : 50, y : 50},{ id : "btnsound", x : 1768, y : 1269},{ id : "btnmusic", x : 1944, y : 1269},{ id : "progressBar", x : 500, y : 20},{ id : "brandingNeobird", x : 196, y : 1199},{ id : "brandingZibbo", x : 541, y : 1269},{ id : "preloadTitle", x : 517, y : 133},{ id : "startTitle", x : 517, y : 133},{ id : "startBtnStart", x : 892, y : 592},{ id : "startBtnLeft", x : 824, y : 799},{ id : "startBtnRight", x : 1366, y : 799},{ id : "startDiffBack", x : 885, y : 804},{ id : "startDiffText", x : 1138, y : 852},{ id : "startBtnHelp", x : 892, y : 997},{ id : "startBtnMoregames", x : 892, y : 1189},{ id : "helpBtnMain", x : 575, y : 1241},{ id : "helpBtnStart", x : 1218, y : 1241},{ id : "helpTopic", x : 1136, y : 134},{ id : "helpText", x : 475, y : 433},{ id : "helpZibbo", x : 203, y : 1277},{ id : "gameBtnNewGame", x : 1639, y : 810},{ id : "gameBtnDiffLeft", x : 1599, y : 964},{ id : "gameBtnDiffRight", x : 1996, y : 964},{ id : "gameDiffBack", x : 1658, y : 942},{ id : "gameDiffText", x : 1829, y : 964},{ id : "gameBtnExit", x : 1601, y : 1269},{ id : "gamePlayfield", x : 171, y : 124},{ id : "gameWellDone", x : 1540, y : 160},{ id : "gameZibbo", x : 1679, y : 1110},{ id : "playfieldTile", x : 136, y : 136},{ id : "playfieldBoard", x : 34, y : 30},{ id : "playfieldSelection", x : 79, y : 42},{ id : "messagepanelBtn", x : 0, y : -10}];
